...
> 3. the newest `versioning/*_v*.md` records — what shipped last and what must not regress.
...
>
> Lessons bible: the intended entry point is the external orchestrator
> `…/HenrisForge/lessons-system/lessons_orchestrator.md` (read it FIRST, append new lessons to the
> correct PART/section). NOTE: in this cloud execution environment that path is **not reachable** —
> only this repo is mounted. When running here, capture the round's lessons in the version record's
> "Lessons captured" block instead, and sync them into the external orchestrator from a machine that
> can see it.
...
- **Implementation constraints** (no-regression, additive-only, single-owner-per-file, honesty,
  changelog/versioning/lessons duties, "CI is the compiler — dl.google.com is unreachable locally");
...
- **Builds green on CI** (`.github/workflows/android.yml` → `assembleDebug` succeeds; unit tests pass).
  CI is the only compiler — `dl.google.com` is blocked in the dev container, so never claim "compiles"
  without a green CI run id.
...
- **Data-safety invariants hold**: copy/move refuses a same-path self-overwrite; every permanent delete is
  confirmed; the vault stays LOCKED without a real biometric host; Smart Merge keeps the highest-quality copy.
