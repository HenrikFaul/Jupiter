...
// Strictness bands map to Hamming thresholds on the 64-bit dHash (research-backed):
// ≤2 near-identical, ≤5 strong, ≤12 similar, ≤20 loose.
const BANDS: { label: string; threshold: number; hint: string }[] = [
  { label: "Exact", threshold: 2, hint: "near-identical (re-saved / resized)" },
  { label: "Strong", threshold: 5, hint: "clearly the same picture" },
  { label: "Similar", threshold: 12, hint: "minor edits / crops / filters" },
...
export function SimilarImages() {
  ...
  const [result, setResult] = useState<SimilarImageResult | null>(null);
  ...
  async function detect() {
    ...
    setOutcome(null);
    try {
      const r = await detectSimilarImages(BANDS[band].threshold);
      setResult(r);
      setDecisions(new Map()); // default: keep everything (similar ≠ identical — you review)
    } catch (e) {
      setError(String(e)); // fail loud
    } finally {
      setBusy(false);
    }
  }
  ...
  // Keep the suggested keeper in every group, mark the rest for removal.
  ...
  return (
    <div className="p-6 space-y-4">
      <header>
        <h1 className="text-xl font-semibold">Similar images</h1>
        <p className="text-muted">Find visually-similar photos — resized, re-saved, cropped, or filtered versions of the same shot — by perceptual hash.</p>
      </header>
      ...
      <Card>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button className="btn btn-primary ml-2" onClick={detect} disabled={busy}>
            {busy ? "Working…" : result ? "Re-detect" : "Detect similar images"}
          </button>
        </div>
        <p className="text-muted text-xs mt-2">{BANDS[band].hint}. Hashes are cached, so changing strictness re-detects fast.</p>
        {error && <div role="alert" className="text-danger text-sm mt-2">Detection failed: {error}</div>}
        {result && (
          <p className="text-sm mt-2">
            {result.groups.length} similar group(s) · {result.totalHashed.toLocaleString()} images hashed
        ...
        <>
          ...
          {outcome && (
            <div role="status" className={`rounded-lg border p-3 shadow-sm ${outcome.state === "success" ? "border-ok/50 text-ok" : outcome.state === "partial" ? "border-warn/50 text-warn" : "border-danger/50 text-danger"}`}>
              {outcome.state === "success" && <p>Removed {outcome.removed} images. Reclaimed {bytes(outcome.reclaimedBytes)}.</p>}
          ...
          <div className="space-y-3">
            {result.groups.map((g: SimilarImageGroup) => (
              <Card key={g.groupId}>
                <div className="text-muted text-xs mb-2">{g.memberCount} similar images · up to {bytes(g.maxSizeBytes)}</div>
                  ...
                  {g.members.map((m) => {
                    ...
                    return (
                      <div key={m.file.fileId} className={`w-[150px] rounded-lg border p-2 ${keep ? "border-border" : "border-danger/60 bg-danger/5"}`}>
                        <Thumbnail fileId={m.file.fileId} size={132} />
                        <div className="flex items-center justify-between mt-1.5">
                          <span className={`chip ${m.isRepresentative ? "online" : ""}`} title="visual similarity to the largest image">
      ...
      {result && result.groups.length === 0 && (
        <Card><p className="text-muted">No similar images found at this strictness. Try a looser band, or run a scan first to index your photos.</p></Card>
      )}
    </div>
  );
}
