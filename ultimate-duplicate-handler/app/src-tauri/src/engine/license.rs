...
// ===========================================================================
// TIER-SPLITTING CAPABILITY (initiative #1). The mechanism is fully built; ENFORCEMENT is
// OFF by default, so NOTHING is gated today (zero regression for current users). Flip
// `ENFORCEMENT_ENABLED` to true at commercial launch to activate the Free/Pro paywall — no
// other code changes needed; the gates are already wired through every Pro command.
// ===========================================================================

/// Master switch for the paywall. `false` = every feature is unlocked for everyone (the
/// current behavior). `true` = features in `PRO_FEATURES` require a valid Pro license. Flipping
/// this single constant ships monetization.
pub const ENFORCEMENT_ENABLED: bool = false;

/// The feature keys that belong to the Pro tier (the subscription split). Free keeps the core
/// scan → review → safe-delete loop fully usable. Move a feature behind Pro by adding its key.
pub const PRO_FEATURES: &[&str] = &[
    "similar_images",  // perceptual similar-image detection
    "similar_videos",  // perceptual similar-video detection
    "media_analysis",  // ffprobe integrity + technical-quality analysis
    "bulk_actions",    // bulk delete/rename in Index Explorer
    "smart_selection", // smart-select rule engine
];

impl Tier {
    /// Ordering rank for entitlement comparisons (Free < Pro).
    fn rank(self) -> u8 {
        match self {
            Tier::Free => 0,
            Tier::Pro => 1,
...
/// Is the current install entitled to `feature`? With enforcement OFF this is ALWAYS true, so
/// every command that calls it keeps working exactly as before until launch day.
pub fn is_entitled(data_dir: &Path, feature: &str) -> bool {
    if !ENFORCEMENT_ENABLED {
        return true;
    }
    load_entitlement(data_dir).rank() >= required_tier(feature).rank()
}

/// Read the stored, re-verified license payload (None if unlicensed/invalid/expired).
fn load_payload(data_dir: &Path) -> Option<LicensePayload> {
    let bytes = std::fs::read(license_path(data_dir)).ok()?;
    let stored: StoredLicense = serde_json::from_slice(&bytes).ok()?;
    verify(&stored.token).ok()
}

/// Full entitlement status for the Settings → License card.
pub fn status(data_dir: &Path) -> crate::model::LicenseStatus {
    let payload = load_payload(data_dir);
    let tier = payload.as_ref().map(|p| p.tier()).unwrap_or(Tier::Free);
    crate::model::LicenseStatus {
        tier: tier.as_str().to_string(),
        email: payload.as_ref().map(|p| p.email.clone()),
        expires_at: payload.as_ref().and_then(|p| p.expires_at),
        enforcement_enabled: ENFORCEMENT_ENABLED,
        pro_features: PRO_FEATURES.iter().map(|s| s.to_string()).collect(),
    }
}
...
mod tests {
    ...
    #[test]
    fn entitlement_unlocked_while_enforcement_off() {
        let dir = std::env::temp_dir();
        assert!(!ENFORCEMENT_ENABLED, "ship with the paywall OFF (zero regression)");
        for f in PRO_FEATURES {
            assert!(is_entitled(&dir, f), "feature {f} must be unlocked while enforcement is off");
        }
        assert_eq!(required_tier("similar_images"), Tier::Pro);
        assert_eq!(required_tier("scan"), Tier::Free);
    }
