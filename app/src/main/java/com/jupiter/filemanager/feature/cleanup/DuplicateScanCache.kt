...
class DuplicateScanCache @Inject constructor(
    ...
    private fun writeSnapshot(groups: List<DuplicateGroup>) {
        runCatching {
            val sb = StringBuilder().append(SNAPSHOT_HEADER).append('\n')
            for (group in groups) {
    ...
    private fun readSnapshot(): List<DuplicateGroup>? {
        return runCatching {
            ...
            if (lines.firstOrNull() != SNAPSHOT_HEADER) {
                // v1 did not persist exact-vs-similar and could resurrect unsafe old media groups
                // as exact cleanup results. Fail closed and let the silent fresh scan replace it.
                file.delete()
                return null
            }
            val byHash = LinkedHashMap<Pair<String, Boolean>, MutableList<FileItem>>()
            lines.drop(1).forEach { line ->
                val p = line.split('\t')
                if (p.size < 8) return@forEach
                val hash = p[0]
                val similar = p[1].toBooleanStrictOrNull() ?: return@forEach
                val path = p[2]
                val name = p[3]
                val size = p[4].toLongOrNull() ?: return@forEach
                val mtime = p[5].toLongOrNull() ?: return@forEach
                val type = runCatching { FileType.valueOf(p[6]) }.getOrDefault(FileType.OTHER)
                byHash.getOrPut(hash to similar) { mutableListOf() }.add(
                    FileItem(
                        path = path,
                        name = name,
                        isDirectory = false,
            ...
            byHash.entries
                ...
                .map {
                    DuplicateGroup(
                        hash = it.key.first,
                        files = it.value.toList(),
                        similar = it.key.second,
                    )
                }
        }.getOrNull()
    }
