...
For v0.54, `assembleDebug`, `testDebugUnitTest`, and `lintDebug` completed
successfully in one local gate; the XML reports contain 344 tests with zero
failures, errors, or skips.

Release gates should repeat device-dependent checks with representative data and
permissions. The v0.54 API 34 emulator smoke verified install/cold launch, the
advertised-capacity storage card, all five suspended Automation examples, mutation-free
preview, the restored duplicate controls, and a real scanned duplicate arrival that produced
a `Duplicate detected` notification. Earlier v0.51 runtime checks covered the eight reference
