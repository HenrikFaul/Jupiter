...
| **Storage and cleanup** | The primary-volume total/free figures use Android's retail-capacity `StorageStatsManager` source and decimal display units, with a safe `StatFs` fallback, so a nominal 256 GB device is not presented as only its smaller user-data filesystem. Real on-device analysis, large-file review, and separate exact/similar duplicate presentations remain data-backed. The duplicate hero totals exact and similar item paths together; always-visible size thresholds, largest/smallest ordering, Select all, and Deselect all operate only on the visible scope while every quality-ranked keeper remains protected. Android 11+ arrival catch-up uses MediaStore version + `GENERATION_MODIFIED` and excludes pending rows; a bounded observer over common user-write folders and an immediate durable WorkManager backup cover OEM/provider timing gaps. A transient unreadable file never advances the checkpoint. Canonical decision keys and a persistent notification outbox keep replayed alerts idempotent while retrying decisions that Android had blocked; **Duplicate alert settings** opens the system control surface. Quick/perceptual/structural hashes never replace the full-hash exact-match decision. |
...
For v0.55, a clean cache-disabled build/test gate produced 59 suites / 354 JVM
tests with zero failures, errors, or skips. `lintDebug` was regenerated too: it
adds no issue in the duplicate-arrival changes, while the report retains the
pre-existing Media3 opt-in and two Compose findings configured not to abort
release builds.

Release gates should repeat device-dependent checks with representative data and
permissions. The v0.55 API 34 emulator smoke verified a byte-identical new Download copy produces
