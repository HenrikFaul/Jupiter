...
| **Storage and cleanup** | The primary-volume total/free figures use Android's `StorageStatsManager` source and decimal display units, with a safe `StatFs` fallback. Raw binary GiB allocations are normalized to the matching retail tier (for example, `256 GiB` is displayed as `256 GB`), while a non-standard OEM capacity remains platform-backed. Real on-device analysis, large-file review, and separate exact/similar duplicate presentations remain data-backed. The duplicate hero totals exact and similar item paths together; always-visible size thresholds, largest/smallest ordering, Select all, and Deselect all operate only on the visible scope while every quality-ranked keeper remains protected. Opening Duplicate cleanup explicitly requests the bounded image-descriptor backfill even when continuous background indexing is off; transient reads retry and only a decoder-confirmed unsupported image is excluded. Android 11+ arrival catch-up uses MediaStore version + `GENERATION_MODIFIED` and excludes pending rows; a bounded observer over common user-write folders and an immediate durable WorkManager backup cover OEM/provider timing gaps. A transient unreadable file never advances the checkpoint. Canonical decision keys and a persistent notification outbox keep replayed alerts idempotent while retrying decisions that Android had blocked; **Duplicate alert settings** opens the system control surface. Quick/perceptual/structural hashes never replace the full-hash exact-match decision. |
