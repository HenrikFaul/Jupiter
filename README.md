...
| **Storage and cleanup** | Real on-device storage analysis, large-file review, and separate exact/similar duplicate presentations. The index is kept fresh by MediaStore signals, foreground catch-up, and a 15-minute WorkManager refresh; completed scans warm exact-duplicate hash candidates and MediaStore delta markers are recorded in the Room `index_state` row. A shared readiness model exposes metadata, exact-hash, image-descriptor, and structural-descriptor coverage. The Similar tab includes photo perceptual groups plus text/code, archive/APK, video, PDF, and audio descriptor groups once their background fingerprints are available. Arrival duplicate alerts are persisted through canonical decision keys so observer/restart replays do not notify twice for the same pair/group. Selection is restricted to the currently visible filter scope, hidden selections are pruned, and every quality-ranked `BEST` keeper remains protected before Recycle Bin deletion. Quick/perceptual/structural hashes never replace the full-hash exact-match decision. |
...
For v0.53, `assembleDebug`, `testDebugUnitTest`, and `lintDebug` completed
successfully in one local gate; the XML reports contain 320 tests with zero
