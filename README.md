...
| **Storage and cleanup** | The primary-volume total/free figures use Android's `StorageStatsManager` source and decimal display units, with a safe `StatFs` fallback. The Storage Truth card explicitly separates platform capacity/free space, shared files Jupiscan can analyse, and used space outside that scan; it never pretends the remainder is all app data. Real on-device analysis, large-file review, and separate exact/similar duplicate presentations remain data-backed. The duplicate hero totals exact and similar item paths together and explains which are byte-identical versus review-only similar photos; the keeper is protected, the reclaimable total is visible, and reviewed cleanups go to Recycle Bin. Size thresholds, largest/smallest ordering, Select all, and Deselect all operate only on the visible scope while every quality-ranked keeper remains protected. Android 11+ arrival catch-up uses MediaStore version + `GENERATION_MODIFIED` and excludes pending rows; a bounded observer over common user-write folders and an immediate durable WorkManager backup cover OEM/provider timing gaps. A transient unreadable file never advances the checkpoint. Canonical decision keys and a persistent notification outbox keep replayed alerts idempotent while retrying decisions that Android had blocked; tapping an alert opens the real duplicate-review queue. Quick/perceptual/structural hashes never replace the full-hash exact-match decision. |
